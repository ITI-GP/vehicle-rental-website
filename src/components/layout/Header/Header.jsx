...
import { supabase } from "../../../Lib/supabaseClient";
import { useState, useEffect } from "react";
import { FaUserTie, FaUsers, FaSignOutAlt } from "react-icons/fa";
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate("/RentYourVehicle")}
                  className="bg-primary text-white px-3 py-1 rounded-md flex items-center gap-1 text-sm"
                >
                  <FaUserTie /> {t("header.seller")}
                </button>
                {/* <button
                  onClick={() => navigate("/vehicles")}
                  className="bg-primary text-white px-3 py-1 rounded-md flex items-center gap-1 text-sm"
                >
                  <FaUsers /> {t("header.buyer")}
                </button> */}
                <button
                  onClick={handleSignOut}
                  className="text-primary hover:text-gray-500 text-xl ml-2"
                  title="Logout"
                >
                  <FaSignOutAlt />
                </button>
        ...
        <div className="md:hidden px-4 pb-4 max-w-7xl mx-auto">
          <div className="flex flex-col items-center gap-4">
            ...
            {user ? (
              <>
                <button
                  onClick={() => { setMenuOpen(false); navigate("/RentYourVehicle"); }}
                  className="bg-gradient-to-r from-orange-400 to-orange-600 text-white px-3 py-1 rounded-md flex items-center gap-1 text-sm"
                >
                  <FaUserTie /> {t("header.seller")}
                </button>
                {/* <button
                  onClick={() => { setMenuOpen(false); navigate("/vehicles"); }}
                  className="bg-gradient-to-r from-orange-400 to-orange-600 text-white px-3 py-1 rounded-md flex items-center gap-1 text-sm"
                >
                  <FaUsers /> {t("header.buyer")}
                </button> */}
                <button
                  onClick={handleSignOut}
                  className="text-primary hover:text-gray-500 text-xl"
                  title="Logout"
                >
                  <FaSignOutAlt />
                </button>
