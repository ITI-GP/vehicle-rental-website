...
import { supabase } from "../../../Lib/supabaseClient"; // عدل المسار لو مختلف
import { useAuth } from "../../../contexts/AuthContext"; // عدل المسار لو مختلف
...
export default function ReviewsSection({ vehicleId }) {
  ...
  const [showAddReviewForm, setShowAddReviewForm] = useState(false); // حالة إظهار النموذج
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setNewComment("");
      setNewRating(5);
      setShowAddReviewForm(false); // إخفاء النموذج بعد الإضافة
      fetchReviews();
    } catch (error) {
